Pour ajouter un sélecteur de langue directement sur la page Kanban, il faut faire deux choses : ajouter un <select> dans l'en-tête HTML et modifier légèrement la logique JavaScript pour que le changement recharge instantanément les traductions depuis l'objet retourné par SQLite.
<template>
...
// 🌟 Variable pour suivre la langue sélectionnée localement
const selectedLang = ref('fr')
// 🌟 Stockage temporaire de la config brute reçue de SQLite pour éviter les requêtes BDD inutiles
...
/**
 * Applique les titres et les couleurs aux colonnes en fonction d'une langue donnée
 */
const applyConfiguration = (config, lang) => {
  const activeLang = lang.toLowerCase().trim()
...
/**
 * Lit la configuration SQLite (Couleurs + Langue enregistrée par défaut)
 */
const loadCustomKanbanConfig = async () => {
  try {
    const config = await kanbanConfigService.fetchConfig()
    cachedConfig = config // On garde en cache
    
    // On initialise le sélecteur avec la langue officielle de la BDD
    selectedLang.value = config.currentLang || 'fr'
    
    applyConfiguration(cachedConfig, selectedLang.value)
  } catch (error) {
...
/**
 * 🌟 Se déclenche quand l'utilisateur change de langue à la main
 */
...
<style scoped>
/* Conserve ton CSS d'origine + les ajouts pour l'alignement */
...
/* Styles pour intégrer proprement le select à côté du bouton */
.header-actions { display: flex; align-items: center; gap: 20px; }
.lang-selector-container { display: flex; align-items: center; gap: 8px; font-weight: 600; color: #475569; font-size: 0.95rem; }
.lang-select { padding: 10px 14px; border: 1px solid #cbd5e1; border-radius: 6px; background-color: #ffffff; font-weight: 600; color: #1e293b; outline: none; cursor: pointer; }
.lang-select:focus { border-color: #1e293b; }
...
.card-date { font-size: 0.8rem; color: #94a3b8; border-top: 1px solid #f1f5f9; padding-top: 10px; }
