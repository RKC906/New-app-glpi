<template>
 ...
 <h3> Zone de Danger - Administration API Directe</h3>
...
const clearGlpiModule = async (endpoint) => {
 ...
 console.log(` ${items.length} éléments trouvés dans ${endpoint}. Début de la suppression...`)
 ...
 console.error(` Impossible de nettoyer entièrement le module : ${endpoint}`, error)
 }
}
...
const handleDirectGlpiReset = async () => {
 ...
 const secondCheck = confirm(" DERNIER AVERTISSEMENT : Cette opération est irréversible et détruira les liaisons en base de données. Es-tu absolument sûr ?")
